...
class Optimize(ElementwiseProblem):
    def __init__(self, **kwargs):
        super().__init__(n_var=3,
                         n_obj=1,
                         n_constr=1,
                         xl=[6,20,50],  # Lower bounds x = [thickness , amp , p]
                         xu=[50,150,200], # Upper bounds
                         **kwargs)
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        width = 3000
        panel = SinePanel(doc, 400, width, thickness, amp, p, 16)
...
if __name__ == '__main__':
    ...
    # Save to CSV
    np.savetxt("SinePanelHistory_PSO.csv", data, delimiter=", ", fmt='%s', header="F, T, Amp, P, G", comments="")
